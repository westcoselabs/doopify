...
- session hash rollout uses the additive `20260713_session_hash_compatibility` migration; valid legacy plaintext sessions remain only through an explicit valid `SESSION_LEGACY_TOKEN_CUTOFF` and missing/invalid configuration disables legacy compatibility. `npm run db:deploy:safe` is mandatory, catalog-checks target-schema objects, and refuses unknown migration history or the historical destructive migration on every existing schema; see the session and provider/store singleton migration runbooks.
- integration resets require an explicit public-schema acknowledgement plus exact normalized equality between `DATABASE_URL_TEST` and `E2E_DATABASE_URL`; normal Vitest, integration, and Playwright runners scrub live provider, storage, webhook, cloud, and deployment credentials.
