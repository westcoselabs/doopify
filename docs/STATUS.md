...
- session hash rollout uses the additive `20260713_session_hash_compatibility` migration; valid legacy plaintext sessions remain only through `SESSION_LEGACY_TOKEN_CUTOFF`. `npm run db:deploy:safe` is mandatory and refuses the historical destructive migration on every existing schema; see the session and provider/store singleton migration runbooks.
