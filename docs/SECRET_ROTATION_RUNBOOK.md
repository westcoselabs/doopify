...
The environment-only upgrade is a variable rename, not a rotation. Preserve both existing current and previous values. Infrastructure secrets and outbound header/signing values live in deployment environment variables and are rotated there. Legacy tables retained during the rollback window still need the original legacy key; do not contract or rotate it until that window closes.

- Existing encrypted application rows (MFA secrets and digital-download tokens) depend on `DATA_ENCRYPTION_KEY`.
- Deploy the new `DATA_ENCRYPTION_KEY` with the old value in `DATA_ENCRYPTION_KEY_PREVIOUS`; the application writes versioned envelopes and can read legacy/current envelopes during the overlap.
...
- Verify MFA login and digital-download access before removing `DATA_ENCRYPTION_KEY_PREVIOUS`.
