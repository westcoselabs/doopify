...
export default function ShopView({ products, collections, pagination, search }) {
      ...
      <style>{`
        *, *::before, *::after { box-sizing:border-box;margin:0;padding:0; }
        ...
        @media (max-width:480px) {
          ...
          .shop-grid,.empty-state { padding-left:16px; padding-right:16px; }
      ...
      <div className="shop">
        <nav className="shop-nav">
          ...
          <div className="nav-right">
            <Link prefetch={false} className="nav-link" href="/">Home</Link>
        ...
        {collections.length ? (
          <div className="collection-rail">
            {collections.map(collection => (
              <Link prefetch={false} className="collection-chip" href={`/collections/${collection.handle}`} key={collection.id}>
                {collection.title}
              </Link>
            ))}
          </div>
        ...
        <div className="shop-toolbar">
          <form action="/shop" className="search-wrap">
            ...
            <input
              ...
              defaultValue={search}
            />
            <button type="submit" className="empty-action">Search</button>
          </form>
        ...
        {products.length === 0 ? (
          <div className="empty-state">
            ...
            <div className="empty-actions">
              {hasSearchQuery ? (
                <Link prefetch={false} className="empty-action" href="/shop">Clear search</Link>
              ) : null}
              <Link prefetch={false} className="empty-action" href="/collections">Browse collections</Link>
            </div>
          </div>
        ) : (
          <div className="shop-grid">
            {products.map(product => {
              ...
              return (
                <Link prefetch={false} className="p-card" href={`/shop/${product.handle}`} key={product.id}>
                  <div className="p-img">
                    <div className="p-img-inner">
                      {image
                        ? <Image alt={product.title} src={image} width={800} height={800} sizes="(max-width: 600px) 100vw, (max-width: 1000px) 50vw, 25vw" />
        ...
        <CatalogPagination pagination={pagination} pathname="/shop" search={search} />
      </div>
    </>
  );
}
